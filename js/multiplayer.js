...
function connect(){
    ...
    socket.on("joinedGroup", function (info){
        ...
        if(username == info["player1"]){
        	$("#sendQuestion, #but-clear, #suggest").removeClass("d-none disabled");
            ...
            adjustButton(true);
           	...
           	$("#sendQuestion, #but-clear, #suggest").addClass("d-none");
    ...
    socket.on("RescheckWord", function(data, prd){
        if(data == false){
            ...
            $("#sendQuestion, #but-clear, #suggest").removeClass("disabled");
            showNoty(false, text);
        }else{
            if(iscoordinator){
                sendWordCoord(prd);
    ...
    socket.on("receiveWordCoord", function(data){
        ...
        if(us == data.sender){
            ...
            $("#sendQuestion, #but-clear, #suggest").addClass("disabled");
    ...
    socket.on("coordinatorLedCreated", function(room){
        ...
        $(".screen").addClass("d-none");
        $("#gameScreen, #menu, #alphas, #sendQuestion, #but-clear, #suggest").removeClass("d-none");
        $("#menu").show();
...
function youarenext(userb){
            $("#specButHolder, #sendQuestion, #but-clear, #suggest").removeClass("d-none disabled");
            $("#alphas").removeClass("d-none");
            adjustButton(true);
}
...
function fillAlphas(){
    for(var i = 0; i < alphas.length; i++){
        ...
        $(letBut).on("click", function(){
            ...
            if(typed.length == maxLetterAllowed){
                testAnim(".warn", "shake");
                return;
            }
            typed += t;
    ...
    //add two buttons
        var send = $('<div id="sendQuestion" class="btn btn-success specBut d-none" onclick="checkWord()">Send</div>');
        //var ans =  $('<div id="ansQuestion" class="btn specBut btn-dark" onclick="sendAnswer()">Answer Question</div>');
        $("#specButHolder").append(send);
        var suggest = $('<div id="suggest" class="btn btn-dark specBut" onclick="randomWord()">Suggest</div>');
        $("#specButHolder").append(suggest);
}
...
function goMenu(){
    ...
    $(".screen").addClass("d-none");
        $("#theBtnHolders").removeClass("d-none");
        $(".multBut, #playWithOnline, #sendQuestion, #but-clear, #suggest").removeClass("disabled");
        $("#synonymsM").empty();
...
function randomWord(){
    //var maxLetterAllowed2 = 8;
    var ranNum = getRndInteger(3, maxLetterAllowed);
    var maxLen = gameWords[ranNum].length;
    var pick = getRndInteger(0, maxLen);
    var picked = gameWords[ranNum][pick];
    picked = picked.split(":")[0];
    
    if($("#prevCont").length > 0){
        var num = $("#prevCont").find(".word-" + picked).length;
            if(num == 0){
                $("#typer").text(picked);
                return;
            }else{
