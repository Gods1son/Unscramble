...
function oneWord(complex, num, lev){
	...
	for(var c = 0; c < splitSyns.length; c++){
		...
		var cSysns = "<div class='eachSyn " + ccolor + "'><div class='eachSynChild'>" + eachSyns.trim() + "</div></div>";
        m+=1;
...
function showWin(coins, word, completed){
    ...
    $("#corrAnswer").empty();
    setTimeout(function(){
    var modHeight = $("#exampleModal").find(".modal-content").eq(0).height();
    var modWidth = $("#exampleModal").find(".modal-content").eq(0).width();
    var modTop = $("#exampleModal").find(".modal-content").eq(0).css("top");

    var canvas = document.getElementById("confetti-canvas");
    canvas.setAttribute("style", "display:block;z-index:999999;pointer-events:none;position:fixed;top:" + modTop);
    canvas.width = modWidth;
	canvas.height = modHeight;
    ...
    //$(".contButPar").hide();
    $(".wonCoins").text(coins);
    if(parseInt(coins) > 0 && !completed){
        coinAnim();
    }
    $("#congratsScreen").css("top", "-100vh");
    if(completed){
        var from = $("#coinNew");
        var coin = $('<img class="coin" src="icon/singleCoin.svg" style="width: 1em;">');
        $(from).append(coin);
    ...
    if(completed){
        ...
        confetti.start(400, 300, 500);
    }
        
    },500)
}

function hideWin(comp){
...
function coinAnim(){
    //$("#showModal").click();
    var num = parseInt($(".wonCoins").text());
    var to = $(".theCoins");
    var from = $("#coinNew");
    if(num == 0){
        return;
...
function loopAnim(){
    ...
    var top = (0 - $('.coin').eq(0).offset().top) + $(to).offset().top + ($(to).height()/2);
    var left = $(to).offset().left/2 - ($(to).width()/3);
        $('.coin').eq(0).addClass("moved");
        $('.coin').eq(0).animate ({
            "top": top + "px",
            "left": left + "px"
        }, 200, 'linear', function() {
            $('.coin.moved').remove();
            var txt = parseInt($(".wonCoins").text()) - 1;
            $(".wonCoins").text(txt);
            var txt2 = parseInt($(".getCoins").text()) + 1;
            $(".getCoins").text(txt2);
            //loopAnim();
        });
        setTimeout(function(){
...
function loopAnim2(num){
    var to = $(".theCoins").find("img").eq(0);
    var top = (0 - $('.coin').eq(0).offset().top) + $(to).offset().top + $(to).height()/4.5;
    var left = $(to).offset().left/2 - ($(to).width()/6) - 5;
    //var top = (0 - $('.coin').eq(0).offset().top) + $(to).offset().top + ($(to).height()/4);
    //var left = $(to).offset().left/2 - ($(to).width()/5);
    anime({
        targets: '.coin',
        top: top + 'px',
        left: left + 'px',
        ...
        complete: function(anim) {
               $('.coin').remove();
               var txt = parseInt($(".wonCoins").text()) - num;
                $(".wonCoins").text(txt);
                var txt2 = parseInt($(".getCoins").text()) + num;
                $(".getCoins").text(txt2);
            }
